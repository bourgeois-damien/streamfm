...
def _instantiate_backbone(cfg, dotted_path: str):
    if dotted_path not in {"dnn", "wrapped_model.dnn"}:
        raise ValueError(
            f"backbone path must be 'dnn' or 'wrapped_model.dnn', got {dotted_path!r}."
        )
...
def compress_checkpoint(
    *,
    ...
    """Decoupled-SVD on selected backbones; writes a self-describing inference checkpoint."""
    ...
    # mmap full training ckpts so we don't RAM the optimizer state during compression
