...
def _is_sensitive_env_key(key: str) -> bool:
    upper = key.upper()
    return (
        any(fragment in upper for fragment in _SENSITIVE_ENV_FRAGMENTS)
        or upper == "KEY"
...
def main() -> None:
    ...
    for name, template, default_out in (
        ...
        ("metal-system-trace", "Metal System Trace", "outputs/benchmark_profiles/macos_metal.trace"),
    ):
    ...
    if args.action in {"time-profiler", "cpu-counters", "metal-system-trace"}:
        template = {
