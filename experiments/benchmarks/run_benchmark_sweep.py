...
def resolve_sweep_iterations(command: dict[str, Any], *, input_audio_path: str) -> int:
    ...
    if iterations == -1:
        # whole-file mode: warmup eats the start of the file
...
def _run_benchmark_modal(command: dict[str, Any], *, input_audio_path: str, output_json: Path) -> list[dict]:
    ...
    modal_command = [
        ...
        "--tf32",
        str(command.get("tf32", "auto")),
        "--trt-optimization-level",
        str(command.get("trt_optimization_level", 3)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="One Stream.FM benchmark trial (wandb sweep agent).")
    parser.add_argument("--hardware", default="", help="Override hardware")
    parser.add_argument("--backend", default="", choices=("local", "modal"), help="Override backend")
