...
def _apply_ptq_int8_if_requested(
    ...
    *,
    ...
    if device.type != "cpu":
        raise ValueError("PTQ INT8 needs CPU (--hardware cpu).")
    if use_compiled:
        raise ValueError("PTQ INT8 needs --execution eager.")
    fallback_dtype = model_dtype
    if fallback_dtype not in {torch.float16, torch.float32}:
        print(f"warn: INT8 fallback {fallback_dtype} -> float32", flush=True)
    ...
    print(
        f"PTQ INT8 ok components={list(components)} "
...
def _benchmark_flow_task(
    *,
    ...
    # ModelOpt calib stays FP32; --dtype is only TRT's float fallback, I/O stays FP32
...
def _benchmark_se_full_task(
    *,
    ...
    # SE PTQ: quantize predictor and flow, not just one of them
...
def run_internal_benchmark(
    *,
    ...
    if use_tensorrt and internal_task not in {"stftpr", "bwe", "derev", "lyra"}:
        raise ValueError("tensorrt only supports flow tasks (stftpr/bwe/derev/lyra).")

    if quality is not None and internal_task not in {"stftpr", "bwe", "derev", "lyra"}:
        raise ValueError("quality runs only support flow tasks (stftpr/bwe/derev/lyra).")
...
def run_benchmark(
    *,
    ...
    """One full benchmark run (local CLI and Modal share this).
    Returns result rows; persistence is the caller's job.
    """
    import torch
    ...
    if cudnn_benchmark and is_tensorrt:
        raise ValueError("--cudnn-benchmark does not apply to TensorRT.")
    ...
    # FP8 needs Ada+ (L4/L40S yes; T4/A100 no)
    ...
    if trt_int8 and model_dtype_name.lower() not in {"fp32", "fp16"}:
        raise ValueError("TensorRT INT8/FP8 needs --dtype fp32 or fp16.")
    ...
    if quality is not None and input_audio_path:
        raise ValueError("--quality-split incompatible with --input-audio.")
