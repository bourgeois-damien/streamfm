...
def confirm_gpu_workers(*, hardware_names: list[str], workers: int, trial_count: int) -> bool:
    ...
    try:
        answer = input("Continue with this many GPU workers? [y/N] ").strip().lower()
    except EOFError:
        print("No interactive input; aborting.", flush=True)
        return False
...
def run_sweep_batch(
    *,
    ...
    if backend == "local" and worker_count > 1:
        print("note: --workers ignored for backend=local.", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a full bench sweep grid and log to W&B.")
    ...
    parser.add_argument("--workers", type=int, default=1, help="Parallel Modal containers")
    parser.add_argument("-y", "--yes", action="store_true", help="Skip confirm when workers > 4")
