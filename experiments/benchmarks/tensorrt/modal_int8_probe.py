...
def probe(
    *,
    ...
    if not tensorrt.__version__.startswith("10.9."):
        raise RuntimeError(
            f"need TensorRT 10.9.x + torch-tensorrt==2.7.0, got {tensorrt.__version__}"
        )
    if not hasattr(torch.ops.tensorrt, "quantize_op"):
        raise RuntimeError("torch.ops.tensorrt.quantize_op missing (bad TRT/ModelOpt install).")
    ...
    if mode == "streaming_int8":
        ...
        if calibration_source == "audio":
            ...
            def calibrate(module):
                # white-noise calib = control only
                with torch.inference_mode():
                    state = module.init_state()
                    for _ in range(calibration_steps):
