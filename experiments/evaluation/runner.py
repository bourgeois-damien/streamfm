...
def _apply_execution(model, execution: str):
    ...
    if execution == "cuda_graph":
        raise ValueError("execution=cuda_graph not supported for offline eval.")
...
def _streaming_backbone(model):
    for holder in (model, getattr(model, "wrapped_model", None)):
        ...
        if backbone is not None:
            return backbone
    raise ValueError("no 'dnn' backbone on model; need --pipeline streaming.")
...
def _prepare_streaming_flow(
    *,
    ...
    tensorrt_quant_scope: str,
    tensorrt_quant_coverage: float,
    ...
    # INT8: ModelOpt calib + engine I/O stay FP32; --dtype is the TRT float fallback only
    ...
    adapter = build_tensorrt_streaming_adapter(
        ...
        quant_scope=tensorrt_quant_scope,
        quant_coverage=tensorrt_quant_coverage,
...
def run_test_set_inference(
    *,
    ...
    tensorrt_calibration_steps: int = 96,
    tensorrt_quant_scope: str = "all",
    tensorrt_quant_coverage: float = 0.8,
    ...
    """Offline inference on a configured dataset split."""
    import torch
    import torchaudio
    ...
    if pipeline == "streaming":
        if execution not in {"eager", "compiled", "tensorrt"}:
            raise ValueError("streaming eval: execution must be eager|compiled|tensorrt.")
    elif execution == "tensorrt":
        raise ValueError("tensorrt needs --pipeline streaming.")
    if execution == "tensorrt":
    ...
    if model_dtype == torch.bfloat16:
        # bf16 breaks complex ops here
        raise ValueError("bf16 unsupported here (view_as_complex); use fp16 or fp32.")
    ...
    # transform BEFORE load when the ckpt was already trained in the pruned shape
    if backbone_pre_transform is not None:
        model.transform_backbone_(backbone_pre_transform)
    model.load_state_dict(checkpoint["state_dict"])
    # transform AFTER load for zero-shot prune (drop blocks, keep teacher weights)
    if backbone_transform is not None:
        model.transform_backbone_(backbone_transform)
    model = model.eval().to(device=device)
    # use autocast; .half() breaks cuFFT/polar
    ...
    if pipeline == "streaming":
        ...
        streaming_flow, streaming_use_engine, model_dtype, streaming_info = _prepare_streaming_flow(
            ...
            tensorrt_quant_scope=tensorrt_quant_scope,
            tensorrt_quant_coverage=tensorrt_quant_coverage,
