...
def run_streaming_audio_pipeline_with_cuda_graph_model(
    ...
    """Eager audio pipeline, but the flow solver is one CUDA Graph replay.
    STFT/ISTFT stay outside (offsets change each frame). Needs fixed-address static buffers.
    """
    ...
    with torch.inference_mode():
        # warmup before capture (autotune/allocs); allocating during capture breaks the graph
        ...
        # replay-vs-eager check so a bad capture shows up in the summary, not as silent wrong audio
...
def run_streaming_audio_pipeline_with_full_cuda_graph(
    ...
    """STFT + compression + solver + ISTFT in one CUDA Graph (one sync/frame instead of several).
    Overlap-add stays outside because the write offset moves. No per-stage timings here.
    """
    assert device.type == "cuda", "CUDA Graph requires a CUDA device."
    assert audio.ndim == 2 and audio.shape[0] == 1, "Expected mono audio shaped [1, T]."
    assert steps > 0
...
def run_streaming_audio_pipeline_with_tensorrt_cuda_graph(
    ...
    """Audio streaming with the TensorRT solver inside one CUDA Graph.
    STFT and overlap-add stay outside.
    """
