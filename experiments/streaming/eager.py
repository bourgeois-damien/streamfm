...
def run_streaming_audio_pipeline(
    ...
    """Frame-by-frame STFTPR: STFT -> flow -> ISTFT.
    Returns timing stats; set return_audio=True to also get the waveform.
    """
    ...
    with torch.inference_mode():
        for frame_idx in range(total_frames):
            ...
            # sync around each stage or the timings are queue-submit, not compute
            sync_device(device)
...
def run_streaming_se_audio_pipeline(
    ...
    """SE streaming path: STFT -> predictor -> flow -> ISTFT.
    Same timing setup as the STFTPR pipeline; conditioning stays complex and flow input is 6-channel.
    """
    assert audio.ndim == 2 and audio.shape[0] == 1, "Expected mono audio shaped [1, T]."
    assert steps > 0
