...
def main(cfg: omegaconf.DictConfig) -> None:
    ...
    if getattr(cfg, 'transform_model_backbone_fn', None) is not None:
        ...
        wrapped_model.transform_backbone_(transform_fn)
        print("[ . ] Transformed model backbone.")

    ckpt = cfg.ckpt
    print(f"Loading checkpoint (*after* transform if any) from {ckpt}")
    wrapped_model = model_load_from_ckpt(wrapped_model, ckpt)

    solver_model = instantiate(cfg.solver_model, wrapped_model=wrapped_model).to(device)
