...
class NCSNpp(nn.Module):
    """NCSN++ model from GitHub sp-uhh/storm repo, slightly adapted for our use cases"""
    ...
    def __init__(self,
        ...
        pad_time_to: int = None,
        pad_dimension_to: int = None,
        **kwargs
    ):
        super().__init__()
        ...
        self.pad_time_to = pad_time_to if pad_time_to is not None else 1
        self.pad_dimension_to = pad_dimension_to if pad_dimension_to is not None else 1
        ...
        temb_dim = nf * 4 if conditional else None
    ...
    def pad_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        *_, F, T = x.shape
        ...
        if T % self.pad_time_to != 0:
            output = torch.nn.functional.pad(output, (0, self.pad_time_to - T % self.pad_time_to))

        # padding across dimension
        if F % self.pad_dimension_to != 0:
            output = torch.nn.functional.pad(output, (0, 0, 0, self.pad_dimension_to - F % self.pad_dimension_to))
        ...
        *_, F, T = x.shape
        ...
        if self.conditional:
            if self.embedding_type == 'fourier':
                ...
                temb = modules[m_idx](used_sigmas)  # no log here
                m_idx += 1
        ...
        h = h[..., :F, :T]  # unpad to original size
        return h
